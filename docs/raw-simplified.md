...
|---|---|---|
| `save_and_quit_to_menu` | _(none)_ | Internal step used by the MCP `sl()` tool. Opens the pause menu, saves and quits to the main menu, and stops there. |

Use the MCP `sl()` tool for the public one-click save/load workflow. It calls `save_and_quit_to_menu`, then uses `menu_select` with `continue`, and blocks until the run is loaded again.
